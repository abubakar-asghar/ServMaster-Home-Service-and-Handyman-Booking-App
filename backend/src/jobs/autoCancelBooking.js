...
export const initScheduledJobs = () => {
  ...
  cron.schedule("*/5 * * * *", async () => {
    ...
    try {
      const now = new Date();
      console.log(`Checking for bookings before ${now.toISOString()}`);
      ...
      for (const request of expiredBookings) {
        try {
          ...
          const chat = await Chat.findOne({
            participants: {
              $all: [
                { $elemMatch: { user: request.customer, participantType: "Customer" } },
                { $elemMatch: { user: request.service_provider, participantType: "ServiceProvider" } },
              ],
            },
              ...
              await Chat.updateOne(
                ...
                { 
                  ...
                  $push: {
                    messages: {
                      sender: "System",
                      message: "This booking was automatically cancelled because the service was not started in time.",
                      sentAt: new Date(),
                      system: true,
                    }
                  }
