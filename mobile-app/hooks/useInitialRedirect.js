...
const useInitialRedirect = () => {
  ...
  useEffect(() => {
    const checkAuthAndOnboarding = async () => {
      try {
        ...
        if (storedAuth?.token) {
          // Immediately set temp route based on stored role while validating
          if (storedAuth.role) {
            setTempRoute(
          ...
          // No token, set to login or onboarding
          setTempRoute(
            onboardingValue === "true" ? "/auth/login" : "/onboarding/step1"
          );
        }
      } catch (error) {
        console.log("Initialization error:", error);
        setTempRoute("/auth/login"); // Fallback to login on error
      } finally {
        setAuthChecked(true);
      }
    };
  ...
  useEffect(() => {
    ...
    const performRedirect = async () => {
      // If we have a temp route (from stored auth), use that immediately
      if (tempRoute) {
        router.replace(tempRoute);
