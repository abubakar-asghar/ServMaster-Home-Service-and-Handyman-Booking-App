...
const MessagesScreen = () => {
  ...
  // Set initial messages and join chat
  useEffect(() => {
    if (data?.data) {
  ...
  const handleSend = () => {
    ...
    try {
      sendMessage(message.trim(), chatId, user.role);
      setMessage("");
    } catch (error) {
  ...
  const renderItem = ({ item }) => {
    const fromMe = item?.senderType === "Customer";
  ...
  if (!selectedChat) {
    return (
      <SafeAreaView className="flex-1 bg-white">
        <TabHeader title="Chat" goBack={customerRoutes.CUSTOMER_HOME} />
        <View className="flex-1 justify-center items-center">
          <Text className="text-muted">Select a chat to continue</Text>
          <TouchableOpacity
            onPress={() => router.replace(customerRoutes.CUSTOMER_CHATS)}
  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      ...
      >
        ...
        >
          {!isConnected && (
            <Text className="text-red-500 text-xs text-center mb-1">
          ...
          <View className="flex-row items-center space-x-3 gap-1">
            <View className="flex-row flex-1 items-center bg-muted-100 rounded-xl">
              <View className="flex-1 px-4 py-2">
                <TextInput
                  ...
                  multiline
                  editable={!isLoadingMessages && isConnected}
            ...
            <TouchableOpacity
              ...
              disabled={!message.trim() || isLoadingMessages || !isConnected}
            >
              <Image
                source={icons.send}
                className="w-7 h-7"
...
//   const flatListRef = useRef(null);
//   const [keyboardHeight, setKeyboardHeight] = useState(0);
//   const { mutate: sendMessageWithSocket } = useSendMessageWithSocket();
//   const {
//     messages,
//     selectedChat,
//     setSelectedChat,
...
//     // Add this new handler for optimistic updates
//     const handleMessageDelivered = (deliveredMessage) => {
//       addMessage({
//         ...deliveredMessage,
//         source: "delivered",
//         _id: deliveredMessage.tempId, // Ensure consistent ID
//       });
//     };
...
//       set((prev) => ({
//         messages: prev.messages.filter((m) => m.tempId !== tempId),
//       }));
//     }
//   };
